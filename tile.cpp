...
void S9xSelectTileRenderers (int BGMode, bool8 sub, bool8 obj)
{
	...
	M7M1 = S9xCurRenderRegs->BGMosaic[0] && S9xCurRenderRegs->Mosaic > 1;
	M7M2 = S9xCurRenderRegs->BGMosaic[1] && S9xCurRenderRegs->Mosaic > 1;

	bool8 interlace = obj ? FALSE : S9xCurRenderRegs->Interlace;
	bool8 hires = !sub && (BGMode == 5 || BGMode == 6 || S9xCurRenderRegs->PseudoHires);

	if (!S9xCurRenderRegs->DoubleWidthPixels)	
	{
	...
	{
		i = (S9xCurRenderRegs->FillRAM[0x31] & 0x80) ? 4 : 1;
		if (S9xCurRenderRegs->FillRAM[0x31] & 0x40)
		{
			i++;
			if (S9xCurRenderRegs->FillRAM[0x30] & 2)
				i++;
		}
		if (S9xCurRenderRegs->MaxBrightness != 0xf)
		{
			if (i == 1)
				i = 7;
			else if (i == 3)
		...
		if (!hires && !(S9xCurRenderRegs->FillRAM[0x30] & 2))
		{
			if (i == 1)
				i = 9;
			else if (i == 4)
...
void S9xSelectTileConverter (int depth, bool8 hires, bool8 sub, bool8 mosaic)
{
	switch (depth)
	{
		case 8:
			...
			BG.PaletteShift     = 0;
			BG.PaletteMask      = 0;
			BG.DirectColourMode = S9xCurRenderRegs->FillRAM[0x30] & 1;
