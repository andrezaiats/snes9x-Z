...
void retro_init(void)
{
    ...
    {
        bool want_threaded = false;

        
        struct retro_variable tvar = { "snes9x_threaded_render", NULL };
        if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &tvar) && tvar.value)
            want_threaded = (strcmp(tvar.value, "enabled") == 0);
        ...
        Settings.ThreadedRender = want_threaded ? TRUE : FALSE;
    }
...
extern "C" RETRO_API uint64_t retro_snes9x_get_instruction_count(void)
{
    return S9xInstructionCounter;
}

extern "C" RETRO_API void retro_snes9x_reset_instruction_count(void)
{
    S9xResetBenchCounters();
}

extern "C" RETRO_API uint64_t retro_snes9x_get_event_count(void)
{
    return S9xEventCount;
}


extern "C" RETRO_API void retro_snes9x_get_block_histogram(uint64_t *out)
{
    for (int i = 0; i < S9X_BLOCK_HISTOGRAM_BUCKETS; i++)
        out[i] = S9xBlockLenHistogram[i];
}
...
extern "C" RETRO_API void retro_snes9x_get_touch_histograms(uint64_t *apu_out, uint64_t *gsu_out,
    uint64_t *scanlines_out, uint64_t *apu_total_out, uint64_t *gsu_total_out)
{
    for (int i = 0; i < S9X_TOUCH_HISTOGRAM_BUCKETS; i++)
    {
        apu_out[i] = S9xAPUTouchHistogram[i];
        gsu_out[i] = S9xGSUTouchHistogram[i];
    }
    *scanlines_out = S9xTouchScanlineCount;
    *apu_total_out = S9xAPUTotalTouches;
    *gsu_total_out = S9xGSUTotalTouches;
}




extern "C" RETRO_API void retro_snes9x_get_gsu_opcode_histogram(uint64_t *out, uint64_t *total_out)
{
    for (int i = 0; i < S9X_GSU_OPCODE_BUCKETS; i++)
        out[i] = S9xGSUOpcodeHistogram[i];
    *total_out = S9xGSUOpcodeTotal;
}
...
extern "C" RETRO_API void retro_snes9x_get_render_thread_histograms(uint64_t *stat77_out, uint64_t *calls_out,
    uint64_t *span_out, uint64_t *scanlines_out, uint64_t *stat77_total_out, uint64_t *calls_total_out,
    uint64_t *span_total_out)
{
    for (int i = 0; i < S9X_TOUCH_HISTOGRAM_BUCKETS; i++)
    {
        stat77_out[i] = S9xSTAT77TouchHistogram[i];
        calls_out[i] = S9xUpdateScreenCallHistogram[i];
        span_out[i] = S9xUpdateScreenSpanHistogram[i];
    }
    *scanlines_out = S9xTouchScanlineCount;
    *stat77_total_out = S9xSTAT77TotalTouches;
    *calls_total_out = S9xUpdateScreenTotalCalls;
    *span_total_out = S9xUpdateScreenTotalSpanLines;
}
