...
static alwaysinline void REGISTER_2104 (uint8 Byte)
{
	if (!(PPU.OAMFlip & 1))
	{
	...
	if (PPU.OAMAddr & 0x100)
	{
		...
		if (Byte != PPU.OAMData[addr])
		{
			...
			int baseObj = (addr & 0x1f) * 4;
			struct SOBJ *pObj = &PPU.OBJ[baseObj];
			...
			S9xRecordOBJ(baseObj);
			S9xRecordOBJ(baseObj + 1);
			S9xRecordOBJ(baseObj + 2);
			S9xRecordOBJ(baseObj + 3);
		}
...
static inline void REGISTER_2122 (uint8 Byte)
{
	if (PPU.CGFLIP)
	{
		if ((Byte & 0x7f) != (PPU.CGDATA[PPU.CGADD] >> 8) || PPU.CGSavedByte != (uint8) (PPU.CGDATA[PPU.CGADD] & 0xff))
		{
			...
			S9xRecordCGRAM(PPU.CGADD, PPU.CGDATA[PPU.CGADD]);
		}
