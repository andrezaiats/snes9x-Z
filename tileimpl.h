...
namespace TileImpl {
	...
	struct Normal1x1Base
	{
		...
		static void FillSpan(uint32 Offset, uint32 OffsetInLine, uint32 Left, uint32 Right, uint8 Pix, uint8 Z2);
		template<int STEP> static void DrawRow8(uint32 Offset, uint32 OffsetInLine, const uint8 *bp, uint8 Z1, uint8 Z2);
		template<int STEP> static void DrawRowClipped(uint32 Offset, uint32 OffsetInLine, const uint8 *bp, uint32 StartPixel, uint32 Width, uint8 Z1, uint8 Z2);
	};
	...
	public:
		...
		alwaysinline void SelectPalette() const
		{
			if (BG.DirectColourMode)
			{
				...
				GFX.RealScreenColors = &RS.ScreenColors[((Tile >> BG.PaletteShift) & BG.PaletteMask) + BG.StartPalette];
	...
	template<class Op>
	struct REGMATH_FIXED
	{
		static alwaysinline uint16 Calc(uint16 Main, uint16 Sub, uint8 SD)
		{
			return Op::fn(Main, GFX.FixedColour);
		}
	};
	typedef REGMATH_FIXED<COLOR_ADD> Blend_AddFixed;
	typedef REGMATH_FIXED<COLOR_SUB> Blend_SubFixed;
	typedef REGMATH_FIXED<COLOR_ADD_BRIGHTNESS> Blend_AddBrightnessFixed;
	...
	template<
		...
		template<class MATH> class PIXEL
	>
	typename Renderers<TILE, PIXEL>::call_t Renderers<TILE, PIXEL>::Functions[12] =
	{
		...
		TILE< PIXEL<Blend_AddFixed> >::Draw,
		TILE< PIXEL<Blend_SubFixed> >::Draw,
		TILE< PIXEL<Blend_AddBrightnessFixed> >::Draw,
	};
	#endif
	...
	#define OFFSET_IN_LINE \
		uint32 OffsetInLine = Offset & (GFX.RealPPL - 1);
	...
	struct DrawTile16
	{
		...
		static void Draw(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount)
		{
			...
			const int	vstep = (Tile & V_FLIP) ? -8 * Pitch : 8 * Pitch;

			bp = (Tile & V_FLIP) ? cache.Ptr() + 56 - bpstart_t::Get(StartLine)
			                     : cache.Ptr() + bpstart_t::Get(StartLine);
			OFFSET_IN_LINE;

			if (Tile & H_FLIP)
			{
				for (l = LineCount; l > 0; l--, bp += vstep, Offset += GFX.PPL)
					PIXEL::template DrawRow8<-1>(Offset, OffsetInLine, bp + 7, Z1, Z2);
			}
			else
			{
				for (l = LineCount; l > 0; l--, bp += vstep, Offset += GFX.PPL)
					PIXEL::template DrawRow8<1>(Offset, OffsetInLine, bp, Z1, Z2);
			}
		}
	};
	...
	struct DrawClippedTile16
	{
		...
		static void Draw(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine, uint32 LineCount)
		{
			...
			{
				for (l = LineCount; l > 0; l--, bp += vstep, Offset += GFX.PPL)
					PIXEL::template DrawRowClipped<-1>(Offset, OffsetInLine, bp + 7 - StartPixel, StartPixel, Width, Z1, Z2);
			}
			else
			{
				for (l = LineCount; l > 0; l--, bp += vstep, Offset += GFX.PPL)
					PIXEL::template DrawRowClipped<1>(Offset, OffsetInLine, bp + StartPixel, StartPixel, Width, Z1, Z2);
			}
		}
	};
	...
	struct DrawBackdrop16
	{
		...
		static void Draw(uint32 Offset, uint32 Left, uint32 Right)
		{
			...
			GFX.RealScreenColors = RS.ScreenColors;
			GFX.ScreenColors = GFX.ClipColors ? BlackColourMap : GFX.RealScreenColors;
			...
			for (l = GFX.StartY; l <= GFX.EndY; l++, Offset += GFX.PPL)
				PIXEL::FillSpan(Offset, OffsetInLine, Left, Right, Pix, Z2);
		}
	};
	...
	struct DrawMode7BG1_OP
	{
		...
		static uint8 DCMODE() { return S9xCurRenderRegs->FillRAM[0x30] & 1; }
	};
	...
	struct DrawTileMosaic
	{
		...
		static void Draw(uint32 Left, uint32 Right, int D)
		{
			...
			if (S9xCurRenderRegs->BGMosaic[0])
			{
				...
				MosaicStart = ((uint32) GFX.StartY - S9xCurRenderRegs->MosaicStart) % VMosaic;
				StartY -= MosaicStart;
			}

			if (S9xCurRenderRegs->BGMosaic[OP::BG])
			{
