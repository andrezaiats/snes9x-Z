...
void S9xReportButton (uint32 id, bool pressed)
{
	...
	map<uint32, s9xcommand_t>::iterator	it = keymap.find(id);

	if (it == keymap.end())
		return;

	s9xcommand_t	&cmd = it->second;

	if (cmd.type == S9xNoMapping)
		return;

	if (maptype(cmd.type) != MAP_BUTTON)
	{
		fprintf(stderr, "ERROR: S9xReportButton called on %s ID 0x%08x\n", maptypename(maptype(cmd.type)), id);
		return;
	}

	if (cmd.type == S9xButtonCommand)	
		if (cmd.button_norpt == pressed)	
			return;

	cmd.button_norpt = pressed;

	S9xApplyCommand(cmd, pressed, 0);
}
	...
	switch (cmd.type)
	{
		...
		case S9xButtonJoypad:
			...
			else
			{
				...
				if (data1)
				{
					if (!Settings.UpAndDown && !S9xMoviePlaying()) 
					{
